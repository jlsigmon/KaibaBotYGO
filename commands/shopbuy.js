                    ...
                    con.query(`SELECT * FROM points WHERE id = '${msg.author.id}'`, (err, rows) => {
                        ...
                        if (rows.length > 0) {
                            if (rows[0].points >= cardCost) {
                                ...
                                con.query(`SELECT * FROM collection WHERE userId = '${msg.author.id}' AND cardName = "${cardName}" AND cardRarity = "${cardRarity}"`, (err, rows) => {
                                    if (err) throw err;
                                    ...
                                    if (rows.length > 0) {
                                        ...
                                        sql3 = `UPDATE collection SET cardNum = ${total} where userId = "${msg.author.id}" AND cardName = "${cardName}" AND cardRarity = "${cardRarity}"`;
                                        con.query(sql3,console.log);
                                        msg.reply("You have successfully bought a " + cardName + "!");
                                    } else {
                                        let sql2;
                                        let totalAdd = 1;
                                        sql2 = `INSERT INTO collection (userId, cardName, cardRarity, cardNum) VALUES ('${msg.author.id}', "${cardName}", "${cardRarity}", ${totalAdd})`;

    ...
    execute(msg, con, Discord){
        ...
        con.query(`SELECT * FROM ranked ORDER BY level DESC, ratio DESC, wins DESC`, (err, rows) => {
            ...
            for(let p = per * (page - 1); p < per * page && p < rows.length; p++){
                if(p < rows.length){
                    pages[page - 1] += (p+1) + ". " + rows[p].name + ", Points: " + rows[p].level + ", Wins: " + rows[p].wins + ", Win Rate: " + (rows[p].ratio) + "\n"; 
                }
            }
                ...
                msg.react('◀').then(r => {
                    ...
                    ford.on('collect', r => {
                        ...
                        for (var i = per; i < per + 10 && i < rows.length; i++) {
                            if(i < rows.length){
                                pages[page - 1] += (i+1) + ". " + rows[i].name + ", Points:" + rows[i].level + ", Wins: " + rows[i].wins + ", Win Rate: " + (rows[i].ratio) + "\n"; 
                            }
                        }
                        per += 10;
                        embed.setDescription(pages[page - 1]);
